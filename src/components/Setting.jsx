...
export default function Setting(props) {
	...
	useEffect(() => {
		props.match.params.id &&
			axios
				.get(
					`${process.env.REACT_APP_BACKEND_API_URL}setting/${props.match.params.id}`,
				)
				.then((result) => {
				...
				<div className={classes.paper}>
					<form
						...
						<Grid container spacing={2}>
							<Grid item xs={12} >
								<h5 class="s-heading">Store Name</h5>
								<TextField
									autoComplete="fname"
									name="Storename"
									variant="outlined"
								...
								{errors &&
									errors
										.filter((err) => err.context.key === "Storename")
										.map((error) => (
											<p className="form-errors">{error.message}</p>
										))}
							</Grid>
							<Grid item xs={12} sm={6}>
							<h5 class="s-heading">Discount</h5>
								<TextField
									variant="outlined"
									fullWidth
									placeholder="                                                                                                                   %"
